...
############################### Efimenko 190121
	iss_number = ""
	iss_user= ""
	iss_subject = issue.subject
	iss_subject.split do |word|
		if %r|[0-9]+| =~ word
			iss_number = word[1..]
		end
	end		

	body = cleaned_up_text_body
	body.each_line do |lll|
		if lll.include?('Заявитель:')
			iss_user = lll[10..].strip
		end
	end
  info = "\""+body[body.index("Информация:")+11..].gsub("\n","")+"\""

  issue_subject = ""+iss_number+" "+iss_user+" "+info
  issue.subject = issue_subject[0..254]
###############################
